...
function relaxPreviewFrameHeaders(currentHeaders: Headers): Headers {
    currentHeaders.delete('X-Frame-Options');

    const csp = currentHeaders.get('Content-Security-Policy');
    if (csp) {
        const sanitized = csp
        ...
        if (sanitized) {
            currentHeaders.set('Content-Security-Policy', sanitized);
        } else {
            currentHeaders.delete('Content-Security-Policy');
        }
    }
...
async function handleUserAppRequest(request: Request, env: Env): Promise<Response> {
	...
	if (sandboxResponse) {
        ...
        headers = relaxPreviewFrameHeaders(headers);
