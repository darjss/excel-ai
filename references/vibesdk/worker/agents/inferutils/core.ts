...
export async function getConfigurationForModel(
    ...
    const hasGatewayToken = !!(
        (gatewayOverride?.token && gatewayOverride.token.trim() !== '') ||
        (env.CLOUDFLARE_AI_GATEWAY_TOKEN &&
            env.CLOUDFLARE_AI_GATEWAY_TOKEN !== 'none' &&
            env.CLOUDFLARE_AI_GATEWAY_TOKEN.trim() !== '')
    );
    const hasDirectGoogleKey = !!(
        env.GOOGLE_AI_STUDIO_API_KEY &&
        env.GOOGLE_AI_STUDIO_API_KEY.trim() !== ''
    );

    // For local/dev setups that only provide a Google AI Studio key, bypass AI Gateway entirely.
    if (modelConfig.provider === 'google-ai-studio' && hasDirectGoogleKey && !hasGatewayUrl && !hasGatewayToken) {
        return {
            baseURL: 'https://generativelanguage.googleapis.com/v1beta/openai/',
            apiKey: env.GOOGLE_AI_STUDIO_API_KEY,
        };
...
export async function infer<OutputSchema extends z.AnyZodObject>({
    ...
    try {
        ...
        // Google's OpenAI-compatible endpoint expects bare model ids like "gemini-2.5-flash",
        // not our internal ids like "google-ai-studio/gemini-2.5-flash".
        if (
            baseURL.startsWith('https://generativelanguage.googleapis.com/') &&
            modelName.startsWith('google-ai-studio/')
        ) {
