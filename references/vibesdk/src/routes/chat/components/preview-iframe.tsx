	...
	({ src, className = '', title = 'Preview', shouldRefreshPreview = false, manualRefreshTrigger, webSocket }, ref) => {
		...
		const testAvailability = useCallback(async (url: string): Promise<'sandbox' | 'dispatcher' | null> => {
			try {
				...
				const isCrossOriginPreview = previewUrl.origin !== window.location.origin;

				// In local dev, custom preview domains may reject the CORS/HEAD probe even though
				// the iframe URL itself is reachable. Fall back to loading the iframe directly.
				if (isLocalHostPage && isCrossOriginPreview) {
					console.log('Preview probe blocked, assuming sandbox preview is reachable in iframe:', error);
					return 'sandbox';
				}
