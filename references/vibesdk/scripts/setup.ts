...
class SetupManager {
	...
	private async updateWranglerConfig(resources: ResourceInfo): Promise<void> {
		...
		if (wranglerConfig.dispatch_namespaces && resources.dispatchNamespaces.length > 0) {
			const hasAccessibleDispatchNamespace = resources.dispatchNamespaces.some(dispatch => dispatch.accessible);

			if (hasAccessibleDispatchNamespace) {
				...
				const dispatchEdits = modify(updatedContent, ['dispatch_namespaces'], undefined, {
					formattingOptions: { insertSpaces: true, tabSize: 4 }
				});
				updatedContent = applyEdits(updatedContent, dispatchEdits);

				const dispatchNamespaceVarEdits = modify(updatedContent, ['vars', 'DISPATCH_NAMESPACE'], undefined, {
					formattingOptions: { insertSpaces: true, tabSize: 4 }
				});
				updatedContent = applyEdits(updatedContent, dispatchNamespaceVarEdits);
			}
		}
	...
	private async generateReadinessReport(resources: ResourceInfo): Promise<ReadinessReport> {
		...
		if (resources.dispatchNamespaces.length > 0 && !resources.dispatchNamespaces.some(dispatch => dispatch.accessible)) {
			suggestions.push('Dispatch namespaces are unavailable, so local dev will start without dispatcher-backed preview or deploy support');
		}

		const localDevReady = issues.length === 0;
		const deploymentReady = localDevReady
			&& this.config.customDomain !== 'localhost:5173'
			&& resources.dispatchNamespaces.every(dispatch => dispatch.accessible);
