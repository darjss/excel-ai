...
Platform that turns a business's existing spreadsheet into a hosted, customer-facing order portal. See `CONTEXT.md` for the domain language and `docs/adr/` for the architecture decisions.

Built on the `saas-starter` platform base: Astro 7 SSR + SolidJS islands + Elysia API (typed end to end via Eden) + better-auth on D1 + Polar billing + Tailwind v4 with the Zaidan/Kobalte component registry, plus the Flue agent harness on the Cloudflare Agents SDK for Extraction.
